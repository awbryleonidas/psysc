	...
	*/
	...
	public function insert($data=NULL)
	{
		...
		if ($this->set_created === TRUE && $this->log_user === TRUE && !array_key_exists($this->created_by_field, $data))
		{
			...
			$data[$this->created_by_field] = $this->session->userdata('user_id');
		}
	...
	public function insert_batch($data=NULL)
	{
		...
		if ($this->set_created === TRUE && $this->log_user === TRUE)
		{
			...
			$set[$this->created_by_field] = $this->session->userdata('user_id');
		}
	...
	public function update($id=NULL, $data=NULL)
	{
		...
		if ($this->set_modified === TRUE && $this->log_user === TRUE && !array_key_exists($this->modified_by_field, $data))
		{
			...
			$data[$this->modified_by_field] = $this->session->userdata('user_id');
		}
			...
			if ($this->set_modified === TRUE && !array_key_exists($this->modified_field, $data))
			{
				foreach ($data as $key => $record)
				{
					...
					if ($this->log_user === TRUE && !array_key_exists($this->modified_by_field, $data[$key]))
					{
						...
						$data[$key][$this->modified_by_field] = $this->session->userdata('user_id');
					}
				}
			}
		...
		if ($this->find($id) !== FALSE)
		{
			if ($this->soft_deletes === TRUE)
			{
				...
				if ($this->log_user === TRUE && !array_key_exists($this->deleted_by_field, $data))
				{
					...
					$data[$this->deleted_by_field] = $this->session->userdata('user_id');
				}
	...
	public function delete_where($data=NULL)
	{
		...
		if ($this->soft_deletes === TRUE)
		{
			...
			{
				$where[$this->deleted_by_field] = $this->session->userdata('user_id');
			}
			...
			if ($this->log_user === TRUE)
			{
				$this->db->update($this->table, array(
					...
					$this->deleted_by_field => $this->session->userdata('user_id'),
				));
			}
			else
			{
...
class MY_Model extends YL_Model {
	...
	public function audit_log($message = NULL)
	{
		if ($message != NULL)
	...
	public function insert($data=NULL)
	{
		...
		if ($status != FALSE)
		{
			...
			// log this action
			$this->auditlog->insert($this->table, $insert_id, lang('action_add'), lang('action_success'), FALSE, $res);

			// $audit = Array(
			// 	'audittrail_table'			=> $this->table,
			// 	'audittrail_action'			=> lang('action_add'),
			// 	'audittrail_message'		=> $this->log_message,
			// 	'audittrail_new'			=> json_encode($res),
		...
		{
			// log this action
			$this->auditlog->insert($this->table, FALSE, lang('action_add'), lang('action_failed'));
	...
	public function insert_batch($data=NULL)
	{
		...
		else
		{
			foreach($data as $entry)
			{
				...
				$res = $this->db->limit(1)->get($this->table)->row();

				// log this action
				// $this->auditlog->insert($this->table, lang('action_add'), lang('action_success'), FALSE, $res);
	...
	public function update($id=NULL, $data=NULL)
	{
		...
		if ($qry->num_rows() > 0)
		{
			...
			if ($this->db->update($this->table, $data))
			{
				...
				// log this action
				$this->auditlog->insert($this->table, $id, lang('action_edit'), lang('action_success'), $old_data, $new_data);
			...
			// log this action
			$this->auditlog->insert($this->table, $id, lang('action_edit'), lang('action_failed'), $old_data, $new_data);
	...
	public function update_where($field=NULL, $value=NULL, $data=NULL)
	{
		...
		if ($return)
		{
			foreach($old_data as $key => $old)
			{
				...
				{
					// log this action
					$this->auditlog->insert($this->table, FALSE, lang('action_edit'), lang('action_success'), $old, $qry->row());
					// $audit = Array(
					// 	'audittrail_table'			=> $this->table,
					// 	'audittrail_action'			=> lang('action_edit'),
					// 	'audittrail_message'		=> $this->log_message,
					// 	'audittrail_old'			=> json_encode($old),
					// 	'audittrail_new'			=> json_encode($qry->row()),
					// 	'audittrail_ip'				=> $this->session->userdata('ip_address'),
					// 	'audittrail_user_agent'		=> $this->session->userdata('user_agent'),
					// 	'audittrail_user_id'		=> $this->session->userdata('user_id'),
					// );
		...
		if ($this->find($id) !== FALSE)
		{
			if ($this->soft_deletes === TRUE)
			{
				...
				if ($qry->num_rows() > 0)
				{
					...
					// log this action
					$this->auditlog->insert($this->table, $id, lang('action_delete'), lang('action_success'), $old_data);

					// $audit = Array(
					// 	'audittrail_table'			=> $this->table,
					// 	'audittrail_action'			=> lang('action_delete'),
					// 	'audittrail_message'		=> $this->log_message,
					// 	'audittrail_old'			=> json_encode($old_data),
					// 	'audittrail_new'			=> json_encode($data),
					// 	'audittrail_ip'				=> $this->session->userdata('ip_address'),
					// 	'audittrail_user_agent'		=> $this->session->userdata('user_agent'),
					// 	'audittrail_user_id'		=> $this->session->userdata('user_id'),
					// );
	...
	public function delete_where($data=NULL)
	{
		...
		if ($qry->num_rows() > 0)
		{
			...
			// // log this action
			// $this->auditlog->insert($this->table, FALSE, lang('action_delete'), lang('action_success'), $old_data);
...
//			$this->db->get($this->table);
			...
			if ($this->log_user === TRUE)
			{
				$update[$this->deleted_by_field] = $this->session->userdata('user_id');
			}
			...
			$audit['audittrail_new'] = json_encode($qry->row());
	...
	public function datatables($fields = Array(), $result_callback = FALSE, $rearrange_fields=FALSE)
	{
		if (is_array($fields) && count($fields) == 0)
		{
			$not_included = Array(
				$this->created_field,
				$this->created_by_field,
				$this->modified_field,
				$this->modified_by_field,
				$this->deleted,
				$this->deleted_field,
				$this->deleted_by_field
			);
	...
	// private function _audittrail($table, $action, $message, $old = FALSE, $new = FALSE, $userid = FALSE)
	// {

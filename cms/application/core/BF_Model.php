...
class BF_Model extends CI_Model
{
	 ...
	 * 
	 */
	// public function datatables($fields = array())
	// {
	// 	$aColumns = $fields;
	...
	// 	// log_message('debug', print_r($rResult, TRUE));
	...
	public function datatables($fields = Array(), $result_callback = FALSE, $rearrange_fields=FALSE)
	{
		if (is_array($fields) && count($fields) == 0)
		{
			...
			foreach($this->db->list_fields($this->table_name) as $fld)
			{
		...
		// select
		$this->db->select('SQL_CALC_FOUND_ROWS '.str_replace(' , ', ' ', implode(', ', $aColumns)), false);
		$rResult = $this->db->where($this->deleted_field, 0)->get($this->table_name);
		...
		// total rows
		$iTotal = $this->db->count_all($this->table_name);
	...
	private function _audittrail($event, $data)
	{
		// log these events
		$events = array('before_insert', 'before_update', 'after_update', 'before_delete');

		if (in_array($event, $events))
		{
			$action = explode('_', $event);
			$data = array(
				'auditlog_action'			=> $action[1],
				'auditlog_data'				=> json_encode($data),
				'auditlog_user_ip'			=> $this->input->ip_address(),
				'auditlog_user_agent'		=> $this->input->user_agent(),
				'auditlog_created_by' 		=> $this->session->userdata('user_id'),
				'auditlog_created_on' 		=> date('Y-m-d H:i:s'),
			);

			$this->db->insert('auditlogs', $data);

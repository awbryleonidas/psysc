...
class Configs extends CI_Controller 
{
	...
	public function index() 
	{
		...
		// render the output
		$this->template->add_css(module_css('settings', 'configs_index'));
		$this->template->add_js(module_js('settings', 'configs_index'));
	...
	public function datatables() 
	{
		...
		$fields = array('config_id', 'config_name', 'config_value');

		echo $this->configs_model->datatables($fields);
	}
	...
	public function form($action, $id = 0) 
	{
		...
		$data = array(
			...
			'page_type'    => $action,
			'record'       => ($id ? $this->configs_model->find($id) : '')
		);
		...
		$this->template->add_js(module_js('settings', 'configs_form'));
	...
	private function _save($type = 'add', $id = 0)
	{
		...
		$this->cache->save('app_configs', $configs_arr, 86400);

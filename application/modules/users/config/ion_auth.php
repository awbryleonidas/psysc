...
$config['site_title']                 = $CI->config->item('application_name');       // Site Title, example.com
$config['admin_email']                = $CI->config->item('application_email'); // Admin Email, admin@example.com
$config['default_group']              = 'User';           // Default group, use name
$config['admin_group']                = 'Admin';             // Default administrators group, use name
$config['identity']                   = 'user_username';             // A database column which is used to login with

// Minimum Required Length of Password
$config['min_password_length']        = $CI->config->item('min_password_length');      

// Maximum Allowed Length of Password             
$config['max_password_length']        = $CI->config->item('max_password_length');      
...
$config['user_expire']                = $CI->config->item('user_expire');              

// Extend the users cookies everytime they auto-login
$config['user_extend_on_login']       = ($CI->config->item('user_extend_on_login')) ? TRUE : FALSE;               

// Track the number of failed login attempts for each user or ip.
$config['track_login_attempts']       = ($CI->config->item('track_login_attempts')) ? TRUE : FALSE;    

// The maximum number of failed login attempts.           
$config['maximum_login_attempts']     = $CI->config->item('maximum_login_attempts');  

// The number of miliseconds to lockout an account due to exceeded attempts
$config['lockout_time']               = $CI->config->item('user_lockout_time');

// The number of miliseconds after which a forgot password request will expire. 
// If set to 0, forgot password requests will not expire.
$config['forgot_password_expiration'] = $CI->config->item('forgot_password_expiration');                   
